...
module.exports=async(browser,url)=>{
 ...
 const totals=await page.evaluate(()=>{selected='2030-01';workplaceFilter='all';render();show('dashboard');q('#earningsDetails').open=true;const s=summary(selected),f=forecast(s);return{s:s.gross,minutes:s.minutes,worked:s.workedMinutes,count:s.completed.length,forecast:f.gross,forecastMinutes:f.minutes,cutoff:summary(selected,'all',new Date('2030-01-15'),false).gross}});
