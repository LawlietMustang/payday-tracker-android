...
(async()=>{
 ...
 const browser=await chromium.launch({headless:true});await require('./onboarding.cjs')(browser,url);await require('./interactions.cjs')(browser,url);await require('./design.cjs')(browser,url);const page=await browser.newPage({viewport:{width:360,height:800}});let errors=[];page.on('pageerror',e=>errors.push(e.message));
 ...
 for(const lang of ['en','de'])for(const theme of ['light','dark']){
  ...
  assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth+1),false);assert.equal(await page.evaluate(()=>q('#reminderHelp').getBoundingClientRect().bottom <= q('.mobile').getBoundingClientRect().top),true,'Reminder controls fit above bottom navigation');await page.screenshot({path:`test-results/shift-reminders-${lang}-${theme}.png`,fullPage:true});await page.click('#editShiftReminders');await page.fill('#shiftReminderAmount','7');await page.evaluate(()=>refreshDeviceSettings());assert.equal(await page.inputValue('#shiftReminderAmount'),'7','Native refresh preserves draft');await page.evaluate(()=>handleAppBack());assert.equal(await page.evaluate(()=>shiftReminderConfig().amount),3,'Back cancels draft');
 }
