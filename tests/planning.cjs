...
(async()=>{
 ...
 const browser=await chromium.launch({headless:true});await require('./onboarding.cjs')(browser,url);await require('./round2.cjs')(browser,url);await require('./interactions.cjs')(browser,url);await require('./design.cjs')(browser,url);await require('./shift-status.cjs')(browser,url);const page=await browser.newPage({viewport:{width:360,height:800}});let errors=[];page.on('pageerror',e=>errors.push(e.message));
 ...
 await page.check('#reminderEnabled');assert.equal(await page.evaluate(()=>window.nativeTest.reminder),true);await page.click('#editLoggingReminder');for(const box of await page.locator('.reminder-days input').all())await box.uncheck();await page.click('#reminderForm button');assert.match(await page.locator('#reminderError').innerText(),/weekday/);
 ...
 await page.evaluate(()=>{deviceSection='lock';show('device')});await page.click('#biometricLock');assert.equal(await page.evaluate(()=>window.lockRequested),true);assert.equal(await page.isChecked('#biometricLock'),false);
 await page.evaluate(()=>{deviceSection='widget';show('device')});await page.click('#pinWidget');assert.equal(await page.evaluate(()=>window.pinRequested),true);
 ...
 await page.check('#shiftReminderEnabled');assert.equal(await page.evaluate(()=>window.shiftPayload.enabled),true);await page.click('#editShiftReminders');await page.fill('#shiftReminderAmount','2');await page.selectOption('#shiftReminderUnit','days');await page.click('#shiftReminderForm button[type=submit]');
 ...
 await page.evaluate(()=>renderDeviceSettings());await page.click('#editShiftReminders');await page.selectOption('#shiftReminderScope','selected');await page.check('#shiftReminderChoices input[value="remind-a"]');await page.click('#shiftReminderForm button[type=submit]');assert.deepEqual(await page.evaluate(()=>window.shiftPayload.shifts.map(x=>x.id)),['remind-a']);
 ...
 await page.click('#editShiftReminders');await page.selectOption('#shiftReminderScope','all');await page.selectOption('#shiftReminderUnit','hours');await page.fill('#shiftReminderAmount','3');await page.click('#shiftReminderForm button[type=submit]');
 ...
 for(const lang of ['en','de'])for(const theme of ['light','dark']){
  ...
  assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth+1),false);assert.equal(await page.evaluate(()=>q('#reminderDiagnostics').getBoundingClientRect().bottom <= q('.mobile').getBoundingClientRect().top),true,'Reminder controls fit above bottom navigation');await page.screenshot({path:`test-results/shift-reminders-${lang}-${theme}.png`,fullPage:true});await page.click('#editShiftReminders');await page.fill('#shiftReminderAmount','7');await page.evaluate(()=>refreshDeviceSettings());assert.equal(await page.inputValue('#shiftReminderAmount'),'7','Native refresh preserves draft');await page.evaluate(()=>handleAppBack());assert.equal(await page.evaluate(()=>shiftReminderConfig().amount),3,'Back cancels draft');
 }
 ...
 await page.click('#reminderDiagnostics');assert.match(await page.locator('#shiftDeliveryStatus').innerText(),/Notifications are blocked/);await page.click('#testShiftDelivery');assert.match(await page.locator('#shiftTestResult').innerText(),/Test blocked/);
