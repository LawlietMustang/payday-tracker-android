...
module.exports=async function(browser,url){
 ...
 await page.reload();await page.waitForSelector('#setupName');assert.equal(await page.inputValue('#setupName'),'Evening café');assert.equal(await page.inputValue('#setupWage'),'20');assert.match(await page.locator('#setupCurrency').innerText(),/USD/);
 ...
 const backup=await page.evaluate(()=>validateBackup(backupDocument()));assert.equal(backup.data.settings.currency,'USD');
 ...
 await page.evaluate(()=>localStorage.clear());await page.reload();await page.waitForSelector('#setupNext');await page.click('#setupNext');await page.fill('#setupName','First job');await page.click('#setupNext');await page.click('#setupSkip');await page.click('#setupNext');assert.equal(await page.evaluate(()=>data.shifts.length),0);assert.equal(await page.evaluate(()=>data.settings.taxMode),'germany');
