...
module.exports=async function(browser,url){
 ...
 await page.evaluate(()=>show('appSettings'));await page.click('#openSetup');await page.click('#setupNext');assert.equal(await page.locator('#setupCurrency').isDisabled(),true);await page.click('#setupNext');await page.click('#setupSkip');await page.click('#setupNext');assert.equal(await page.evaluate(()=>data.shifts.length),1);
 // Fresh setup: skip is valid; German estimates remain the existing EUR calculation.
 await page.evaluate(()=>localStorage.clear());await page.reload();await page.waitForSelector('#setupNext');await page.click('#setupNext');await page.fill('#setupName','First job');await page.selectOption('#setupCountry','DE');await page.click('[data-tax-class=IV]');await page.click('#setupNext');await page.click('#setupSkip');await page.click('#setupNext');assert.equal(await page.evaluate(()=>data.shifts.length),0);assert.equal(await page.evaluate(()=>data.settings.taxMode),'germany');assert.equal(await page.evaluate(()=>data.settings.taxclass),'IV');assert.equal(await page.evaluate(()=>data.settings.country),'DE');
 await page.reload();assert.equal(await page.evaluate(()=>data.settings.taxclass),'IV');
 await page.evaluate(()=>{void askConfirm('Restore this backup?','Restore backup')});assert.equal(await page.locator('#confirmDialog .brand-info svg').count(),1);await page.screenshot({path:'test-results/restore-info.png'});await page.click('#confirmCancel');
