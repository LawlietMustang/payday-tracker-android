...
(()=>{
 ...
 function renderOverview(){
  ...
  hero.innerHTML='<div class="available-card"><div class="available-top"><span>'+msg('NACH AUSGABEN VERFÜGBAR','AVAILABLE AFTER EXPENSES')+'</span><span class="estimate-pill">'+msg('GESCHÄTZT','ESTIMATE')+'</span></div><strong id="designAvailable">'+money(available)+'</strong><p>'+msg('Mit geplanten Schichten, minus Monatsausgaben','Includes planned shifts, minus monthly expenses')+'</p></div><div class="overview-pills"><div><small>'+msg('STUNDEN DIESES MONATS','HOURS THIS MONTH')+'</small><b>'+duration(s.workedMinutes)+'</b></div><button type="button" id="designNextShift"><small>'+msg('NÄCHSTE SCHICHT','NEXT SHIFT')+'</small><b>'+ (next?safe(localDate(next.date).toLocaleDateString(locale(),{day:'numeric',month:'short'})+' · '+next.start):msg('Schicht planen →','Plan a shift →'))+'</b></button></div>';
