...
function renderPlanning(){if(!q('#planning'))return;
 ...
 const goals=data.savingsGoals.map(g=>'<article class="panel"><div class="panelhead"><h3 data-localized>'+safe(savingsGoalName(g))+'</h3><button class="secondary" data-goal-edit="'+g.id+'">'+msg('Bearbeiten','Edit')+'</button></div><strong>'+money(g.saved)+' / '+money(g.target)+'</strong>'+meter(g.saved,g.target)+'<p class="muted">'+(g.saved>=g.target?msg('Ziel erreicht','Goal reached'):msg('Noch ','Remaining: ')+money(g.target-g.saved))+(g.due?' · '+safe(g.due):'')+'</p><button class="secondary" data-goal-delete="'+g.id+'">'+msg('Ziel löschen','Delete goal')+'</button></article>').join('');
